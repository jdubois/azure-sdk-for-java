...
public class ConfigurationAsyncClientTest extends ConfigurationClientTestBase {
    ...
    public void updateSettingNullKey() {
        assertRunnableThrowsException(() -> client.updateSetting(null, "A Value").block(),
    ...
    public void updateSettingIfEtag() {
        updateSettingIfEtagRunner(settings -> {
            ...
            StepVerifier.create(client.updateSetting(new ConfigurationSetting().key(last.key())
                .label(last.label()).value(last.value()).etag(initialEtag)))
                .verifyErrorSatisfies(ex -> assertRestException(ex, ResourceNotFoundException.class,
                    HttpURLConnection.HTTP_PRECON_FAILED));
            ...
            StepVerifier.create(client.updateSetting(new ConfigurationSetting().key(last.key())
                .label(last.label()).value(last.value()).etag(updateEtag)))
            ...
            StepVerifier.create(client.updateSetting(new ConfigurationSetting().key(initial.key())
                .label(initial.label()).value(initial.value()).etag(updateEtag)))
                .verifyErrorSatisfies(ex -> assertRestException(ex, ResourceNotFoundException.class,
                    HttpURLConnection.HTTP_PRECON_FAILED));
        });
    }
    ...
    public void deleteSettingNotFound() {
        final String key = getKey();
        final ConfigurationSetting neverDeletedConfiguration = new ConfigurationSetting().key(key).value(
            "myNeverDeletedValue");
        ...
        StepVerifier.create(client.deleteSettingWithResponse(new ConfigurationSetting()
            .key(neverDeletedConfiguration.key()).label("myNonExistentLabel")))
            .assertNext(response -> assertConfigurationEquals(null, response, HttpURLConnection.HTTP_NO_CONTENT))
            .verifyComplete();
    ...
    public void deleteSettingNullKey() {
        assertRunnableThrowsException(() -> client.deleteSetting((String) null).block(),
            IllegalArgumentException.class);
        assertRunnableThrowsException(() -> client.deleteSetting((ConfigurationSetting) null).block(),
    ...
    public void listSettingsAcceptDateTime() {
        ...
        List<ConfigurationSetting> revisions =
            client.listSettingRevisions(new SettingSelector().keys(keyName)).collectList().block();
    ...
    public void listRevisions() {
        ...
        StepVerifier.create(client.listSettingRevisions(new SettingSelector().keys(keyName).fields(SettingFields.KEY,
            SettingFields.ETAG)))
    ...
    public void listRevisionsWithPaginationAndRepeatStream() {
        ...
        for (int value = 0; value < numberExpected; value++) {
            ConfigurationSetting setting =
                new ConfigurationSetting().key(keyPrefix).value("myValue" + value).label(labelPrefix);
        ...
        configurationSettingPagedFlux.toStream()
            .forEach(configurationSetting -> configurationSettingList1.add(configurationSetting));
        assertEquals(numberExpected, configurationSettingList1.size());

        configurationSettingPagedFlux.toStream()
            .forEach(configurationSetting -> configurationSettingList2.add(configurationSetting));
    ...
    public void listRevisionsWithPaginationAndRepeatIterator() {
        ...
        PagedFlux<ConfigurationSetting> configurationSettingPagedFlux = client.listSettingRevisions(filter);
        configurationSettingPagedFlux.toIterable()
            .forEach(configurationSetting -> configurationSettingList1.add(configurationSetting));
        assertEquals(numberExpected, configurationSettingList1.size());
